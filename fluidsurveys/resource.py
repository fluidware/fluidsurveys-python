...
from fluidsurveys import api_requestor, error, util


def convert_to_fluid_object(resp, api_key):
    ...
    if isinstance(resp, list):
        return [convert_to_fluid_object(i, api_key) for i in resp]
    elif isinstance(resp, dict) and not isinstance(resp, FluidObject):
        resp = resp.copy()
        klass_name = resp.get('object')
        if isinstance(klass_name, basestring):
            klass = types.get(klass_name, FluidObject)
        elif util.isclass(klass_name) and issubclass(klass_name, APIResource):
            klass = resp.pop('object')  #remove object from dict so as not to serialize it
        else:
            klass = FluidObject
...
class FluidObject(dict):

    def __init__(self, id=None, api_key=None, **params):
        super(FluidObject, self).__init__()
    ...
    def __setattr__(self, k, v):
        if k[0] == '_' or k in self.__dict__:
            return super(FluidObject, self).__setattr__(k, v)
        else:
            self[k] = v
    ...
    def __setitem__(self, k, v):
        ...
        super(FluidObject, self).__setitem__(k, v)
        self._unsaved_values.add(k)

    def __getitem__(self, k):
        try:
            return super(FluidObject, self).__getitem__(k)
        except KeyError, err:
            if k in self._transient_values:
                raise KeyError(
                    "%r.  HINT: The %r attribute was set in the past."
                    "It was then wiped when refreshing the object with "
                    "the result returned by Fluid's API, probably as a "
    ...
    def __delitem__(self, k):
        raise TypeError(
            "You cannot delete attributes on a FluidObject. "
    ...
    def refresh_from(self, values, api_key=None, partial=False):
        ...
        for k, v in values.iteritems():
            super(FluidObject, self).__setitem__(
                k, convert_to_fluid_object(v, api_key))
    ...
    def request(self, method, url, params=None):
        ...
        return convert_to_fluid_object(response, api_key)
    ...
    def to_dict(self):
        warnings.warn(
            ...
            'version 2.0 of the Fluid bindings. The FluidObject is '
...
class FluidObjectEncoder(util.json.JSONEncoder):

    def __init__(self, *args, **kwargs):
        warnings.warn(
            '`FluidObjectEncoder` is deprecated and will be removed in '
            'version 2.0 of the Fluid bindings.  FluidObject is now a '
            'subclass of `dict` and is handled natively by the built-in '
            'json library.',
            DeprecationWarning)
        super(FluidObjectEncoder, self).__init__(*args, **kwargs)


class APIResource(FluidObject):
    ...
    def instance_url(self):
        ...
        id = unicode(util.utf8(id))
        base = self.class_url()
        extn = urllib.quote_plus(id)
        return "%s/%s" % (base, extn)


class ListObject(FluidObject):
...
class ListableAPIResource(APIResource):
    ...
    def all(cls, api_key=None, **params):
        ...
        #All method "manager" needs modifications to handle FluidSurveys REST
        #and pass 'object' key in response so all objects can be mapped to appropriate class.
        print '@classmethod all for %s' % (cls,)
        results = response['results']
        for r in results:
            r['object'] = cls
        return convert_to_fluid_object(results, api_key)
