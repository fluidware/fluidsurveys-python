...
from fluidsurveys.test.helper import (
    FluidUnitTestCase, FluidApiTestCase,
    MySingleton, MyListable, MyCreatable, MyUpdateable, MyDeletable,
    MyResource, SAMPLE_INVOICE)

from fluid import util


class FluidObjectTests(FluidUnitTestCase):

    def test_initializes_with_parameters(self):
        obj = fluidsurveys.resource.FluidObject(
    ...
    def test_access(self):
        obj = fluidsurveys.resource.FluidObject('myid', 'mykey', myparam=5)
    ...
    def test_refresh_from(self):
        obj = fluidsurveys.resource.FluidObject.construct_from({
            'foo': 'bar',
            'trans': 'me',
        }, 'mykey')
    ...
    def test_refresh_from_nested_object(self):
        ...
        self.assertTrue(
            isinstance(obj.lines.subscriptions[0],
                       fluidsurveys.resource.FluidObject))
...
class ListObjectTests(FluidApiTestCase):

    def setUp(self):
        super(ListObjectTests, self).setUp()

        self.lo = fluidsurveys.resource.ListObject.construct_from({
            'id': 'me',
            'url': '/my/path',
        }, 'mykey')
    ...
    def assertResponse(self, res):
        self.assertTrue(isinstance(res[0], fluidsurveys.Charge))
...
class APIResourceTests(FluidApiTestCase):
    ...
    def test_convert_to_fluid_object(self):
        sample = {
            'foo': 'bar',
            'adict': {
                'object': 'charge',
        ...
        converted = fluidsurveys.resource.convert_to_fluid_object(sample, 'akey')

        # Types
        self.assertTrue(isinstance(converted, fluidsurveys.resource.FluidObject))
        self.assertTrue(isinstance(converted.adict, fluidsurveys.Charge))
        self.assertEqual(1, len(converted.alist))
        self.assertTrue(isinstance(converted.alist[0], fluidsurveys.Customer))
...
class SingletonAPIResourceTests(FluidApiTestCase):
...
class ListableAPIResourceTests(FluidApiTestCase):
    ...
    def test_all(self):
        ...
        self.assertTrue(all(isinstance(obj, fluidsurveys.Charge) for obj in res))
        self.assertEqual('jose', res[0].name)
        self.assertEqual('curly', res[1].name)


class CreateableAPIResourceTests(FluidApiTestCase):
    ...
    def test_create(self):
        ...
        self.assertTrue(isinstance(res, fluidsurveys.Charge))
        self.assertEqual('bar', res.foo)


class UpdateableAPIResourceTests(FluidApiTestCase):
...
class DeletableAPIResourceTests(FluidApiTestCase):
