...
class HttpClientTests(FluidUnitTestCase):

    def setUp(self):
        super(HttpClientTests, self).setUp()

        self.original_filters = fluidsurveys.http_client.warnings.filters[:]
        fluidsurveys.http_client.warnings.simplefilter('ignore')

    def tearDown(self):
        fluidsurveys.http_client.warnings.filters = self.original_filters

        super(HttpClientTests, self).tearDown()

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(fluidsurveys.http_client, lib, None)

        inst = fluidsurveys.http_client.new_default_http_client()

        self.assertTrue(isinstance(inst, expected))

    def test_new_default_http_client_urlfetch(self):
        self.check_default((),
                           fluidsurveys.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(('urlfetch',),
                           fluidsurveys.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(('urlfetch', 'requests',),
                           fluidsurveys.http_client.PycurlClient)

    def test_new_default_http_client_urllib2(self):
        self.check_default(('urlfetch', 'requests', 'pycurl'),
                           fluidsurveys.http_client.Urllib2Client)
...
class ClientTestBase():
    ...
    def valid_url(self, path='/foo'):
        return 'https://api.fluidsurveys.com%s' % (path,)
...
class RequestsVerify(object):
    ...
    def __eq__(self, other):
        return other and other.endswith('fluid/data/ca-certificates.crt')


class RequestsClientTests(FluidUnitTestCase, ClientTestBase):
...
class UrlFetchClientTests(FluidUnitTestCase, ClientTestBase):
    request_client = fluidsurveys.http_client.UrlFetchClient
...
class Urllib2ClientTests(FluidUnitTestCase, ClientTestBase):
...
class PycurlClientTests(FluidUnitTestCase, ClientTestBase):
    ...
    def setUp(self):
        ...
        self.sio_patcher = patch('fluidsurveys.util.StringIO.StringIO')
    ...
    def mock_error(self, mock):
        ...
        fluidsurveys.http_client.pycurl.error = FakeException
        mock.perform.side_effect = fluidsurveys.http_client.pycurl.error
