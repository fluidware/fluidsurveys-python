...
import fluidsurveys
from fluidsurveys import error, http_client, version, util
...
def _api_encode(data):
    for key, value in data.iteritems():
        ...
        if value is None:
            continue
        elif hasattr(value, 'fluid_id'):
            yield (key, value.fluid_id)
...
class APIRequestor(object):
    ...
    def __init__(self, key=None, client=None):
        ...
        from fluidsurveys import verify_ssl_certs
    ...
    def api_url(cls, url=''):
        warnings.warn(
            ...
            'at support@fluidsurveys.com.',
            DeprecationWarning)
        return '%s%s' % (fluidsurveys.api_base, url)
    ...
    def request_raw(self, method, url, params=None):
        ...
        """
        from fluidsurveys import api_version

        if self.api_key:
            my_api_key = self.api_key
        else:
            from fluidsurveys import api_key
            my_api_key = api_key

        if my_api_key is None:
            raise error.AuthenticationError(
                'No API key provided. (HINT: set your API key using '
                '"fluidsurveys.api_key = <API-KEY>"). You can generate API keys '
                'from the Fluid web interface.  See https://fluidsurveys.com/api '
                'for details, or email support@fluidsurveys.com if you have any '
                'questions.')

        abs_url = '%s%s' % (fluidsurveys.api_base, url)
        if abs_url[-1] != '/': abs_url += '/'
        ...
        if method == 'get' or method == 'delete':
            ...
            raise error.APIConnectionError(
                ...
                'Fluid bindings.  Please contact support@fluidsurveys.com for '
                'assistance.' % (method,))
        ...
        headers = {
            'X-Fluid-Client-User-Agent': util.json.dumps(ua),
            'User-Agent': 'Fluid/v1 PythonBindings/%s' % (version.VERSION,),
            'Authorization': 'Bearer %s' % (my_api_key,)
        }

        #added Basic Auth for fluidware.
        headers['Authorization'] = 'Basic %s' % (base64.encodestring('%s:%s' % ('steve@chide.it', 'password')).replace('\n', ''),)

        if api_version is not None:
            headers['Fluid-Version'] = api_version
    ...
    def _deprecated_request(self, impl, method, url, headers, params):
        warnings.warn(
            ...
            ' in `fluidsurveys.http_client` instead',
            DeprecationWarning, stacklevel=2)
    ...
    def requests_request(self, meth, abs_url, headers, params):
        from fluidsurveys.http_client import RequestsClient
    ...
    def pycurl_request(self, meth, abs_url, headers, params):
        from fluidsurveys.http_client import PycurlClient
    ...
    def urlfetch_request(self, meth, abs_url, headers, params):
        from fluidsurveys.http_client import UrlFetchClient
    ...
    def urllib2_request(self, meth, abs_url, headers, params):
        from fluidsurveys.http_client import Urllib2Client
