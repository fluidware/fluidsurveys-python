...
if not (json and hasattr(json, 'loads')):
    try:
        ...
        if not json:
            raise ImportError(
                "Fluid requires a JSON library, such as simplejson. "
                "HINT: Try installing the "
                "python simplejson library via 'pip install simplejson' or "
                "'easy_install simplejson', or contact support@fluidsurveys.com "
                "with questions.")
        else:
            raise ImportError(
                "Fluid requires a JSON library with the same interface as "
                ...
                "or 'easy_install simplejson', or contact support@fluidsurveys.com"
                "with questions.")
